...
import { fadeLeftRight } from "../animations";
...
export default function Pillars() {
    ...
    return (
        <motion.section
            ...
            exit="exit"
            viewport={{ amount: 0.1 }}
            className="flex flex-col sm:flex-row justify-around items-center py-20 bg-black text-white text-center"
        >
            {pillars.map((pillar, index) => (
                <div key={index} className="flex flex-col items-center mb-12 sm:mb-0 group transition-all duration-300 max-w-xs">
                    <div className="w-24 h-24 bg-[#FF007F] rounded-full flex items-center justify-center shadow-[0_0_25px_#ff007f80] group-hover:shadow-[0_0_45px_#ff007f] transform group-hover:scale-110 transition-all duration-300">
                        {pillar.icon}
                    </div>
                    <p className="mt-4 text-xl font-semibold tracking-wide drop-shadow-[0_0_6px_#ff007f80]">{pillar.text}</p>
